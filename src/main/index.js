...
// IPC для изменения спрайта от AI
ipcMain.on('change-emote', (event, emoteType) => {
    // Рассылаем эмоцию
    try {
        event.sender.send('change-emote', emoteType)
    } catch (err) {
        console.error(err)
        event.sender.send('change-emote', 'standart')
    }
})
...
// Функция для изменения эмоции из main process
function changeEmote(emoteType) {
    const windows = BrowserWindow.getAllWindows()
    windows.forEach((win) => {
        win.webContents.send('emote-change', emoteType)
    })
}
